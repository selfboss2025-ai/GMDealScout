...
exports.sendUserResponse = sendUserResponse;
 ...
 */
function formatMinerAnalysis(metric, index) {
 ...
 */
function generateAnalysisMessage(opportunities, username) {
    const now = new Date();
 ...
 */
async function publishAnalysis(bot, opportunities, username) {
    ...
    try {
        await bot.telegram.sendMessage(config_1.config.channelChatId, message, {
            parse_mode: 'HTML',
        });
 ...
 */
async function sendUserResponse(bot, chatId, opportunities) {
